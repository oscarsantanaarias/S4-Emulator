  ...
  internal class AuthService : ProudMessageHandler
  {
    ...
    public async Task EULoginHandler(ProudSession session, LoginEUReqMessage message)
    {
      ...
      using (var db = AuthDatabase.Open())
      {
        ...
        {
          Logger.Information($"Iniciar sesión desde {ip}");
          ...
          if (message.Username.Length > 5 && message.Password.Length > 5)
          {
            if (!Namecheck.IsNameValid(message.Username))
            {
              ...
              Logger.Error("Inicio de sesión incorrecto para {ip}", ip);
              return;
            }
        ...
        {
          Logger.Information("Inicio de sesión de sesión desde { ip}", ip);
          ...
          if (account != null)
          {
            ...
            {
              await session.SendAsync(new LoginEUAckMessage(AuthLoginResult.Failed2));
              Logger.Error("Inicio de sesión incorrecto para {ip} ({AuthToken}, {newToken})", ip,
        ...
        if (ban != null)
        {
          ...
          Logger.Error("{user} está prohibido ingresar hasta {until}", account.Username, unbanDate);
          await session.SendAsync(new LoginEUAckMessage(unbanDate));
          return;
        }

        Logger.Information("Inicio de sesión exitoso para {user}", account.Username);
