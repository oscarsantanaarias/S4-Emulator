...
namespace NeoNetsphere.Network.Services
{
  ...
  internal class RoomService : ProudMessageHandler
  {
    ...
    [MessageHandler(typeof(ScoreKillReqMessage))]
        ...
        {
            var plr = session.Player;
            if (plr == null || plr.Room == null || !plr.Room.HasStarted)
                return;

            var room = plr.Room;

            var target = room.Players.GetValueOrDefault(message.Score.Target.AccountId);
            if (target != null && target.RoomInfo != null)
                target.RoomInfo.PeerId = message.Score.Target;

            var killer = room.Players.GetValueOrDefault(message.Score.Killer.AccountId);
            if (killer != null && killer.RoomInfo != null)
                killer.RoomInfo.PeerId = message.Score.Killer;

            // Aquí se verifica la nulabilidad de assist antes de pasarlo a OnScoreKill
            Player assist = null;  // O asigna un valor válido si es necesario
            room.GameRuleManager.GameRule.OnScoreKill(killer, assist, target, message.Score.Weapon, message.Score.Target, message.Score.Killer, null);
        }
