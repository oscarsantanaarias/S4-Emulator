...
{
    // Verificar si killer, target y assist son jugadores válidos antes de continuar
    ...
    if (realplayerKiller && killer != null)  // Verificar que killer no sea null antes de usarlo
    {
        killer.RoomInfo.Stats.Kills++;
        killer.stats.Kills++;
    }

    if (realplayerTarget && target != null)  // Verificar que target no sea null antes de usarlo
    {
        target.RoomInfo.Stats.Deaths++;
        target.stats.Deaths++;
    }

    if (realplayerAssist && assist != null)  // Verificar que assist no sea null antes de usarlo
    {
    ...
    if (killer != null && killer.PlayerCoinBuff.FindBuff(BuffType.PEN).IsEnabled)  // Verificar que killer no sea null antes de usarlo
    {
        killer.LuckyShot.TryShot(LuckyShotType.PEN);
    }

    if (killer != null && killer.PlayerCoinBuff.FindBuff(BuffType.EXP).IsEnabled)  // Verificar que killer no sea null antes de usarlo
    {
